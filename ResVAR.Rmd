...
# Crear columna con cartera ponderada fija
stock_data <- stock_data %>%
  mutate(cartera = PesoTSLA * TSLA + PesoNVDA * NVDA)

# Calcular rendimientos semanales (5 días) no solapados
secuencia <- seq(1, nrow(stock_data) - 5, by = 5)
R_sem <- vector("numeric", length = length(secuencia))

for(i in seq_along(secuencia)) {
  idx <- secuencia[i]
  R_sem[i] <- log(stock_data$cartera[idx + 5] / stock_data$cartera[idx])
}

# VaR no paramétrico con corrección por la media de los rendimientos
# Se suma el retorno promedio para ajustar el VaR histórico y hacerlo más preciso
VaR_NoParam_Prop <- -Cartera * (quantile(R_sem, 0.01, na.rm = TRUE) + mean(R_sem, na.rm = TRUE))

paste("VaR no paramétrico con proporciones (ajustado):", round(VaR_NoParam_Prop, 2))
...
# Cantidades fijas para cada activo
Cantidades <- c(AccionesTSLA, AccionesNVDA)

# Crear índice multiplicando precios por cantidades fijas
stock_data <- stock_data %>%
  mutate(
    IndiceCarteraCant = TSLA * Cantidades[1] + NVDA * Cantidades[2]
  )

# Usar el mismo intervalo que en j (cada 5 días)
IndiceReducidoCant <- stock_data %>%
  slice(secuencia) %>%
  arrange(date)

# Calcular rendimientos logarítmicos con saltos de 5 días
RendimientosCant <- log(IndiceReducidoCant$IndiceCarteraCant[-1] / IndiceReducidoCant$IndiceCarteraCant[-nrow(IndiceReducidoCant)])

# Valor total actual de la cartera (última observación del índice con cantidades)
ValorCarteraCant <- IndiceReducidoCant$IndiceCarteraCant[nrow(IndiceReducidoCant)]

# VaR no paramétrico con cantidades ajustado por la media de los rendimientos
# Se suma el retorno promedio para ajustar el VaR histórico y hacerlo más preciso
VaR_NoParam_Cant <- -ValorCarteraCant * (quantile(RendimientosCant, 0.01, na.rm = TRUE) + mean(RendimientosCant, na.rm = TRUE))

paste("VaR no paramétrico con cantidades (ajustado):", round(VaR_NoParam_Cant, 2))
